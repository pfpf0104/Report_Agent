...
이 모듈이 계산하는 건 "지난 N년간 이 자산들의 일간 수익률이 실제로 얼마나
같이 움직였는가"라는 검증 가능한 사실뿐이다 — 레짐(성장×인플레 4분면)
분류나 인과관계를 주장하지 않는다. 레짐 분류는 별도 모듈
app/computation/regime/classifier.py가 담당한다(Phase 3-1, 완료) — GDP·CPI
같은 거시지표를 실측 소싱해 판정하며, 이 페이지와 나란히 3개 리포트에
공유 페이지로 붙는다(app/rendering/templates/partials/regime_page.html).
