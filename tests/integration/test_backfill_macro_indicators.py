...
import app.ingestion.jobs.backfill_macro_indicators as job
from app.db.base import SessionLocal
from app.db.models.dim_asset import DimAsset
from app.db.models.fact_market_daily import FactMarketDaily
from app.db.models.ingestion_run import IngestionRun
from app.ingestion.jobs.ingest_macro_indicators import ALL_SERIES
...
def _cleanup(session, codes):
    ...
    session.query(IngestionRun).filter_by(source="backfill_macro_indicators").delete()
    session.commit()
...
def _mock_all_series(rows: list[tuple[str, str, str]] | None = None) -> None:
    default_rows = rows or [
        ("2021-09-15", "2021-08-01", "95.0"),
        ("2026-07-17", "2026-06-01", "102.6"),
    ]
...
def test_backfill_inserts_multiple_months_with_correct_knowledge_date(db):
    ...
    assert len(rows) == 2
    assert rows[0].knowledge_date.isoformat() == "2021-09-15"
    assert rows[1].knowledge_date.isoformat() == "2026-07-17"
...
def test_backfill_skips_fred_missing_value_marker(db):
    ...
    assert len(rows) == 1
    assert float(rows[0].close) == 102.6
