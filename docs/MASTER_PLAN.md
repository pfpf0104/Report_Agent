...
| 4-4 | **산업·경쟁 분석** (밸류에이션) — CFA 필수 섹션 중 현재 누락분. 미착수 |
...
2. **Phase 4 잔여분 — 4-4(산업·경쟁 분석), 4-5(시나리오 확률 근거)** — 4-2·
   4-3은 완료됐다. 4-1(투자의견 헤더박스)은 G3/G4가 실측화되기 전까지
   의도적으로 보류(위 설계 노트 참고).
...
공시 페이지), 4-3(데이터 계보 부록), 5-1(CI), 5-2(스케줄러,
financial_statements 포함), 5-3(모니터링·알림), G13, G4(부분 — 한국·미국
금리 부분).
