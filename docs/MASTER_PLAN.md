...
| 1-2 | **성과 지표** — CAGR, 변동성, Sharpe, Sortino, Calmar, MDD, 회복기간 |
| 1-3 | **리스크 지표** — 히스토리컬/파라메트릭 VaR, CVaR(ES), 하방편차, 베타, 추적오차 |
| 1-4 | **GIPS 성과표** — 5년 이상 연간 수익률 + 벤치마크 병기 + 3년 ex-post 표준편차 |
...
| 작업 | 내용 |
|---|---|
| 2-1 | **비중 산출** — 동일가중 / 역변동성 / 리스크패리티(Bridgewater All Weather 계열) |
| 2-2 | **제약 엔진** — 종목·섹터 상한, 회전율 한도, 유동성 제약 |
| 2-3 | **거래비용 모델** — 스프레드 + 시장충격, 리밸런싱 손익분기 분석 |
| 2-4 | **위험예산(risk budgeting)** — 국민연금식 "위험한도 내 배분" |
| 2-5 | **Brinson 귀속** — 배분효과 vs 선택효과 분해 |
...
| 작업 | 내용 |
|---|---|
| 5-1 | **CI 파이프라인** — 커밋마다 92개 테스트 자동 실행 |
