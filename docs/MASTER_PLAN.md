...
**리포트 반영 — CallRank 완료, 나머지 2종 미완.**
`risk/report_context.py`가 DB → 성과 페이지 연결부다. CallRank는 보류 페이지가
실제 성과·리스크 표 + GIPS 표 + 12개월 롤링 차트로 교체됐다(이력 부족 시에는
보류 페이지로 자동 폴백).

여기서 **백테스트하는 것은 리스크패리티 중립 배분이지 CallRank 전략이 아니다.**
CallRank 점수는 아직 난수라(G3), 그 신호로 기울인 백테스트를 성과로 싣는 것은
제거하기로 한 G2와 같은 물건이 된다. 중립 배분은 규칙이 완전히 명시돼 있고 입력이
실제 가격뿐이라 검증 가능한 사실이며, 그 성격을 공시로 함께 싣는다. G3가 해소되면
같은 엔진에 tilt만 추가하면 전략 백테스트가 된다.

MetroGuard(채권 듀레이션)와 밸류에이션(단일 종목 RIM)은 섹터 ETF 리스크패리티가
그대로 적용되지 않는다 — 각자에 맞는 성과 정의가 필요하며 아직 보류 페이지를 쓴다.
