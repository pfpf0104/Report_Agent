...
| 3-3 | **통합 매크로 대시보드** — 3개 전략을 한 레짐 프레임에서 조망. 3-1이 완료돼 착수 가능해졌으나 아직 미착수(현재는 3개 리포트에 레짐 페이지를 각자 붙인 상태 — "하나의 대시보드로 통합 조망"은 다음 단계) |
| 3-4 | **인과 서술 생성** — "왜 이렇게 움직이는가"를 데이터에서 자동 도출. 미착수 |

신규 리포트 **Macro Regime Observations**(4번째 리포트, 월간)는 검토 대상으로
남아 있다 — 3-1·3-2 모두 완료됐지만 지금은 신규 리포트가 아니라 기존 3개
리포트에 공유 페이지로 붙이는 방식을 택했다(범위를 좁힌 결정, 2026-08).
...
1. **Phase 3-3 통합 매크로 대시보드** — 3-1(레짐 분류)·3-2(크로스에셋
   상관행렬) 둘 다 완료돼 착수 가능. 지금은 3개 리포트 각자에 레짐 페이지·
   상관행렬 페이지를 따로 붙인 상태 — "3개 전략을 한 프레임에서 조망"하는
   통합 뷰(신규 리포트 또는 통합 섹션)가 다음 단계다.
...
3. **Phase 3-4 인과 서술 생성** — 3-1·3-2가 갖춰져 데이터 기반은 있으나
   "왜 이렇게 움직이는가"를 자동 도출하는 로직은 아직 없다.

완료된 항목(재작업 불필요): 0-1~0-5 전부, 1-1~1-5(CallRank·MetroGuard 리포트
반영 포함), 2-1~2-5, 3-1(레짐 분류기), 3-2(크로스에셋 상관행렬), 5-1(CI),
