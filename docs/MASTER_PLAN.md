...
| G4 | ~~MetroGuard City AI가 스텁~~ — **부분 해소**. `global_rate_model.py`가 실제 PCA-8+Ridge λ=10 파이프라인을 구현해 KTB1Y/KTB3Y+미국 금리곡선 15개(원본 49개 중 17개, 주택 지표 미소싱)로 63거래일 KTB3Y 변화를 예측. `city_ai_stub`이 이 예측을 우선하고 이력 부족 시 합성 폴백 | `city_ai_stub.py`, `global_rate_model.py` | ~~높음~~ |
...
않으며, CallRank/MetroGuard는 전략 신호 자체가 아직 합성(G3/G4)이라 "투자
의견"이라는 이름의 강한 주장을 붙이면 G2 원칙과 충돌한다. G3(실제 transcript
임베딩)·G4(주택지표 잔여분)가 해소돼 신호가 실측 기반이 된 뒤에 재검토한다.
...
1. **G4 잔여분 — 주택 지표** — `global_rate_model.py`가 한국 금리+미국
   금리곡선 17개로 실제 PCA-Ridge 예측을 내지만, 원본의 미국 전국·도시
   주택 지표(9개, Zillow ZHVI 등)는 아직 미소싱이다. 학습창도 원본 60개월
   대신 36개월로 축소돼 있다(보유 이력이 5년뿐이라 60개월+63거래일 라벨
   성숙 구간을 채울 수 없음 — 데이터가 누적되면 상수만 올리면 됨).
...
3. **Phase 4는 4-1(투자의견 헤더박스)만 남았다** — G3(실제 transcript
   임베딩)·G4(주택지표 잔여분)가 실측화되기 전까지 의도적으로 보류(위
   설계 노트 참고). Phase 4의 나머지(4-2~4-5)는 전부 완료됐다.
...
5-3(모니터링·알림), G13, G4(부분 — 한국·미국 금리 부분).
