...
| 4-5 | **시나리오 확률 근거 문서화** — 현재 20/50/25/5%는 근거 서술이 정성적. 4-2 공시 페이지에서 이 한계를 명시는 했으나, 확률 자체를 통계적으로 재도출하는 것은 별도 작업으로 남음 |
...
2. **Phase 4 잔여분 — 4-5(시나리오 확률 근거 문서화)** — 4-2·4-3·4-4는
   완료됐다. 4-1(투자의견 헤더박스)은 G3/G4가 실측화되기 전까지 의도적으로
   보류(위 설계 노트 참고).
3. **마이크론 인제스천 라이브 검증** — `ingest_micron_financials.py`는
...
공시 페이지), 4-3(데이터 계보 부록), 4-4(산업·경쟁 분석), 5-1(CI),
5-2(스케줄러, financial_statements 포함), 5-3(모니터링·알림), G13, G4(부분
— 한국·미국 금리 부분).
