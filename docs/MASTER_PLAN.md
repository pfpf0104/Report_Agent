...
| 작업 | 내용 |
|---|---|
| 3-1 | **레짐 분류기** — 성장↑↓ × 인플레↑↓ 4분면 (All Weather 프레임). GDP·CPI 등 거시지표 신규 소싱 필요, 미착수 |
...
| 3-3 | **통합 매크로 대시보드** — 3개 전략을 한 레짐 프레임에서 조망. 3-1(레짐 분류)이 선행돼야 의미가 있어 미착수 |
| 3-4 | **인과 서술 생성** — "왜 이렇게 움직이는가"를 데이터에서 자동 도출. 미착수 |

신규 리포트 **Macro Regime Observations**(4번째 리포트, 월간)는 3-1·3-3이 함께
있어야 의미가 있다 — 지금 완료된 3-2는 신규 리포트가 아니라 기존 3개 리포트에
공유 페이지로 붙였다(범위를 상관행렬 계산/시각화로 좁힌 결정, 2026-08).
...
1. **Phase 3-1 레짐 분류기** — 3-2(크로스에셋 상관행렬)는 완료했으나 GDP·CPI
   같은 거시지표가 DB에 전혀 없어 레짐(성장×인플레 4분면) 분류는 별도
   소싱이 필요하다.
...
반영 포함), 2-1~2-5, 3-2(크로스에셋 상관행렬), 5-1(CI), 5-2(스케줄러,
